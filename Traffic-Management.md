...
A value of zero disables either one of these. While the mesh will function just fine without **any** adverts, it becomes impossible to understand the paths that messages take and thus make informed planning decisions about repeater locations.

### Use Of Regions

Configuring [Regions](Regions.md) is a way of restricting (scoping) how far [Channel](Channels.md) messages propagate. Other traffic such as Adverts (which form a significant proportion of all traffic) will be unaffected.
...
There is also a repeater CLI setting for [path.hash.mode](https://docs.meshcore.io/cli_commands/#region-management-v110) which similarly affects how far its adverts propagate. The following sets the hash mode to 3 bytes (the value is desired size -1):

```
set path.hash.mode 2
```

Note that this is a relatively new setting that requires repeaters with firmware 1.14 or newer. Older repeaters only forward traffic with a hash size of 1. My experience is that around me the majority of repeaters are updated fairly regularly and use of this feature doesn't unduly affect my usage.
...
Messages with a higher hop count can clearly leak in from other peoples repeaters but you will have reduced the volume of mesh traffic around yours.
